...
import type { Pattern, RuleConfig, ValidationContext } from './types.js';
import { daysBetween } from './util.js';

export function computeBannedPatterns(context: ValidationContext, config: RuleConfig): Pattern[] {
  ...
  for (const day of priorDaysInWindow) {
    for (const pattern of day.patterns) counts.set(pattern, (counts.get(pattern) ?? 0) + 1);
  }
