...
export async function fetchContext(
  ...
  const slotResult = await pool.query<{
    id: string;
    date: string;
    day_type: ValidationContext['calendar_slot']['day_type'];
    ...
    `select id, date::text, day_type, target_modalities::text[] as target_modalities, slot_in_block, override_reason,
  ...
  if (!slot) throw new Error(`calendar_slots row not found for id=${calendarSlotId}`);
  if (!slot.training_block_id) throw new Error(`calendar_slots ${calendarSlotId} has no training_block_id -- can't validate Rules 5/7 without one`);

  const blockResult = await pool.query<{
    block_number: number;
    block_type: ValidationContext['training_block']['block_type'];
    lead_modality: Modality | null;
    template_direction: ValidationContext['training_block']['template_direction'];
  }>(
    `select block_number, block_type, lead_modality, template_direction
       from training_blocks where id = $1`,
    [slot.training_block_id],
  );
  const block = blockResult.rows[0];
  if (!block) throw new Error(`training_blocks row not found for id=${slot.training_block_id}`);

  const onDaysResult = await pool.query<{
    date: string;
    slot_in_block: number;
    target_modalities: Modality[];
  }>(
    `select date::text, slot_in_block, target_modalities::text[] as target_modalities
       from calendar_slots
      where training_block_id = $1 and slot_in_block is not null
      order by slot_in_block`,
    [slot.training_block_id],
  );
  ...
  const trailingResult = await pool.query<{ date: string; day_type: TrailingDay['day_type']; patterns: Pattern[] | null }>(
    `select cs.date::text, cs.day_type,
  ...
  const tiesResult = await pool.query<{
    date: string;
    tie_type: PriorTie['tie_type'];
    segment_type: PriorTie['segment_type'];
    placement: PriorTie['placement'];
  }>(
    `select cs.date::text, st.tie_type, ws.segment_type, ws.placement
      ...
      where cs.program_cycle_id = $1
        ...
        and ws.placement is not null`,
    [slot.program_cycle_id, slot.date, tieLookbackDays],
  );
  ...
  if (!profileId) {
    const ownerResult = await pool.query<{ owner_id: string }>(
      `select p.owner_id
        ...
        where pc.id = $1`,
      [slot.program_cycle_id],
    );
    profileId = ownerResult.rows[0]?.owner_id;
  }

  const repMaxResult = profileId
    ? await pool.query<{ movement_name: string; value: number; achieved_at: string }>(
        `select m.canonical_name as movement_name, pr.value, pr.achieved_at::text
    ...
    : { rows: [] as RepMaxRecord[] };

  const movementRows = await pool.query<{ canonical_name: string; alias: string | null; modality: Modality; patterns: Pattern[] }>(
    `select m.canonical_name, ma.alias, m.modality,
            array_remove(array_agg(distinct mp.pattern), null)::text[] as patterns
       from movements m
  ...
  const movementsByCanonical = new Map<string, MovementInfo>();
