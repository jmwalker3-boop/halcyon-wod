...
export type Pattern =
  | 'squat' | 'hinge' | 'lunge' | 'unilateral-leg'
  | 'push-vertical' | 'push-horizontal'
  | 'pull-vertical' | 'pull-horizontal'
  | 'core' | 'carry' | 'jump' | 'monostructural-cardio';
...
// ---------------------------------------------------------------------------
// Context: everything a rule needs, pre-fetched. Rule functions are pure --
// no DB access inside a rule -- so they're unit-testable against plain
// fixtures and the DB-touching part of the engine stays in one place
// (src/context.ts).
// ---------------------------------------------------------------------------
...
export interface TrainingBlockInfo {
   ...
   */
  on_days: { date: string; slot_in_block: number; target_modalities: Modality[] }[];
}
...
export interface RuleConfig {
  [ruleName: string]: Record<string, unknown> & { active?: boolean };
}
