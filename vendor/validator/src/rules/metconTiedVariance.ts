...
function shareViolation(
  ...
  if (totalBefore < minSample) return null; // not enough history to judge a trend
  ...
  if (actualPct - targetPct > tolerancePct) {
    return {
      rule: RULE_NAME,
      message:
        `Adding this ${label} tie (${proposedKey}) would put the year-to-date ${label} split at ${actualPct.toFixed(1)}% ${proposedKey} ` +
...
export function checkMetconTiedVariance(
  ...
  const windowDays = (cfg?.window_days as number) ?? 365;
  const typeSplit = (cfg?.type_split as Record<string, number>) ?? { skill: 50, strength: 50 };
  const placementSplit = (cfg?.placement_split as Record<string, number>) ?? { pre: 70, post: 30 };
  const minSample = (cfg?.min_sample as number) ?? DEFAULT_MIN_SAMPLE;
  const tolerancePct = (cfg?.tolerance_pct as number) ?? DEFAULT_TOLERANCE_PCT;
  ...
  const typeCounts: Record<string, number> = { skill: 0, strength: 0 };
  const placementCounts: Record<string, number> = { pre: 0, post: 0 };
  ...
  const proposedTies = sequence.segments
    ...
    .map((s) => ({ segment_type: s.segment_type as 'skill' | 'strength', placement: s.placement }));
  ...
  for (const tie of proposedTies) {
    ...
    if (tie.placement) {
      const placementViolation = shareViolation(
        'placement',
        placementCounts,
        tie.placement,
        minSample,
      ...
      if (placementViolation) violations.push(placementViolation);
    }
  }
