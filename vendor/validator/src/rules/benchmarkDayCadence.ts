...
export function checkBenchmarkDayCadence(
  ...
  const blockPattern = (cfg?.block_pattern as string[]) ?? ['benchmark', 'standard', 'standard'];
  const onDaysByType = (cfg?.on_days_by_block_type as Record<string, number>) ?? {
    benchmark: 3,
    standard: 3,
  };
