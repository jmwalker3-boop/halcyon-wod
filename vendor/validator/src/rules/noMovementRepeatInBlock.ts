...
export function checkNoMovementRepeatInBlock(
  ...
  const priorUseInBlock = new Map<string, string[]>();
  for (const day of context.block_movements) {
    if (day.date === today) continue; // defensive -- today shouldn't have a committed workout yet at draft time
  ...
  const alreadyFlagged = new Set<string>(); // report each repeated movement once, even if it appears in multiple segments today
  for (const { name } of allMovements(sequence)) {
    const info = resolveMovement(context, name);
    const canonical = info?.canonical_name ?? name;
    if (alreadyFlagged.has(canonical)) continue;
    ...
    if (priorDates && priorDates.length > 0) {
      ...
      violations.push({
        rule: RULE_NAME,
        message:
          `"${canonical}" already appears earlier in this block, on ${priorDates.join(', ')} -- ` +
