...
import type { RuleConfig, RuleViolation, ValidationContext, DraftSequence } from '../types.js';
import { daysBetween, segmentsOfType } from '../util.js';

const RULE_NAME = 'Strength Based on Recent Rep Maxes';

export function checkStrengthRecentRepMax(
  ...
  const recencyWindowDays = (cfg?.recency_window_days as number) ?? 90;

  const violations: RuleViolation[] = [];
  for (const strengthSegment of segmentsOfType(sequence, 'strength')) {
    for (const movement of strengthSegment.movements) {
      if (movement.prescribed_load?.basis !== 'recent_1rm_pct') continue;
