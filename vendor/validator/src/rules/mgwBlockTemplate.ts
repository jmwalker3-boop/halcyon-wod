...
import type { DraftSequence, Modality, RuleConfig, RuleViolation, ValidationContext } from '../types.js';

const RULE_NAME = 'MGW Block Template';

const ORDERS: Record<'ascending' | 'descending', Record<Modality, Modality[]>> = {
  ascending: { M: ['M', 'G', 'W'], G: ['G', 'W', 'M'], W: ['W', 'M', 'G'] },
  descending: { M: ['M', 'W', 'G'], W: ['W', 'G', 'M'], G: ['G', 'M', 'W'] },
};

function sameSet(a: Modality[], b: Modality[]): boolean {
  if (a.length !== b.length) return false;
  const setB = new Set(b);
  return a.every((m) => setB.has(m));
}

export function checkMgwBlockTemplate(
  _sequence: DraftSequence,
  ...
  const exemptBenchmark = (config['Benchmark Day Cadence']?.exempt_from_modality_template as boolean) ?? true;
  if (exemptBenchmark && block.block_type === 'benchmark') return [];
  ...
  const order = ORDERS[block.template_direction as 'ascending' | 'descending'][block.lead_modality as Modality];
  const onDays = [...block.on_days].sort((a, b) => a.slot_in_block - b.slot_in_block);

  for (const day of onDays) {
    const expected: Modality[] =
      day.slot_in_block === 1 ? [order[0]] : day.slot_in_block === 2 ? [order[1], order[2]] : order;
    ...
    if (!sameSet(day.target_modalities, expected)) {
      violations.push({
        rule: RULE_NAME,
        message:
          `Block ${block.block_number} (order ${order.join('')}) expects {${expected.join(', ')}} ` +
          `at slot ${day.slot_in_block} (${label}), but got {${day.target_modalities.join(', ') || 'none'}}.`,
      });

      if (day.slot_in_block === 2 && day.target_modalities.includes(block.lead_modality as Modality)) {
        violations.push({
          rule: RULE_NAME,
