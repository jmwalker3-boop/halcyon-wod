...
import type { DraftSequence, Pattern, RuleConfig, RuleViolation, ValidationContext } from '../types.js';
import { allMovements, daysBetween, resolveMovement } from '../util.js';

const RULE_NAME = '2-of-3 Rule';

export function checkTwoOfThree(
  ...
  const appliesTo = (cfg?.applies_to_day_types as string[]) ?? ['Training', 'Skill', 'Recovery'];
  if (!appliesTo.includes(context.calendar_slot.day_type)) return [];

  const windowDays = (cfg?.window_days as number) ?? 3;
  const maxRepeats = (cfg?.max_repeats as number) ?? 2;
  ...
  const todaysPatterns = new Set<Pattern>();
  for (const { name } of allMovements(sequence)) {
    const info = resolveMovement(context, name);
    if (info) info.patterns.forEach((p) => todaysPatterns.add(p));
  }
  ...
  for (const pattern of todaysPatterns) {
    ...
    if (totalDaysWithPattern > maxRepeats) {
      violations.push({
        rule: RULE_NAME,
        message:
          `Pattern "${pattern}" would appear on ${totalDaysWithPattern} of the trailing ${windowDays} days ` +
